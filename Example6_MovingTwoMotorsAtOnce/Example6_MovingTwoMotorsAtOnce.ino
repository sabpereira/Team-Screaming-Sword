...
// may limit the top speed of each.  This library can generate a maximum of about 
// 7,000 steps per second using an Arduino Uno.  Assuming a system driving only 
// one motor at a time, in full step mode, with a 200 steps per rotation motor, 
// the maximum speed is about 35 RPS or 2100 RPM (most stepper motor can not go 
// this fast).  Driving one motor in half step mode, a maximum speed of 17 RPS 
// or 1050 RPM can be reached.  In quarter step mode about 9 RPS or 525 RPM.  
// Running multiple motors at the same time will reduce the maximum speed.  For 
// example running two motors will reduce the step rate by half or more.
//
...
void loop() 
{
  ...
  stepperX.setTargetPositionRelativeInSteps(200);
  ...
  stepperY.setTargetPositionRelativeInSteps(-200);
